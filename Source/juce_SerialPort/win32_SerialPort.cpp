...
bool SerialPort::open(const String & portPath)
{
	...
	portHandle = CreateFile( (const char*)portPath, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
				...
				if ( dwMask & EV_RXCHAR )
				{
					do
					{
						...
						if(bytesread==1)
						{
							...
							((unsigned char *)buffer)[bufferedbytes]=c;
							bufferedbytes++;
...
int SerialPortInputStream::read(void *destBuffer, int maxBytesToRead)
{
	...
	memcpy(destBuffer, (void*)buffer, maxBytesToRead);
...
void SerialPortOutputStream::run()
{
	...
	while(port && port->portHandle && !threadShouldExit())
	{
		...
		if(bufferedbytes)
		{
			...
			memcpy(tempbuffer, (void*)buffer, bytestowrite);
...
bool SerialPortOutputStream::write(const void *dataToWrite, int howManyBytes)
{
	...
	memcpy((char*)buffer+bufferedbytes, dataToWrite, howManyBytes);
