...
bool SerialPort::open(const String & portPath)
{
	...
	portDescriptor = ::open((const char*)portPath.getCharPointer(), O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (portDescriptor == -1)
    {
...
void SerialPortInputStream::run()
{
	while(port && (port->portDescriptor!=-1) && !threadShouldExit())
	{
		...
		if(bytesread==1)
		{
			...
			((unsigned char *)buffer.getData())[bufferedbytes]=c;
			bufferedbytes++;
...
int SerialPortInputStream::read(void *destBuffer, int maxBytesToRead)
{
	...
	memcpy(destBuffer, (void*)buffer.getData(), maxBytesToRead);
...
void SerialPortOutputStream::run()
{
	...
	while(port && (port->portDescriptor!=-1) && !threadShouldExit())
	{
		...
		if(bufferedbytes)
		{
			...
			memcpy(tempbuffer, (void*)buffer.getData(), bytestowrite);
...
bool SerialPortOutputStream::write(const void *dataToWrite, int howManyBytes)
{
	...
	memcpy((char*)buffer.getData()+bufferedbytes, dataToWrite, howManyBytes);
